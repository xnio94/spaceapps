...
import './globals.dart';
class MyDb {
  Future<void> setDataToDoc(int id,Map data) {
    Document doc = Document(path:'mama_days/'+id.toString());
...
class Document<T> {
  ...
  Future<T> getData() {
    return ref.get().then((v) => Global.models[T](v.data,ref.documentID) as T);
  }

  Stream<T> streamData() {
    return ref.snapshots().map(
          (v) => Global.models[T](v.data,ref.documentID) as T,
        );
    //ref.documentID
  }
...
class Collection<T> {
  ...
  Stream<List<T>> streamData() {
    return ref.snapshots().map(
          (list) => list.documents
              .map(
                (doc) => Global.models[T](doc.data,doc.documentID) as T,
              )
              .toList(),
        );
  }
